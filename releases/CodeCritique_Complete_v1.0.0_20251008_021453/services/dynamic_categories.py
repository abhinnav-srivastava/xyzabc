...
try:
    from utils.path_utils import get_checklists_path, get_config_path, get_project_root
except ImportError:
    ...
    def get_config_path(config_file: str) -> Path:
        return Path(get_base_dir()) / "config" / config_file
...
class DynamicCategoriesService:
    ...
    def __init__(self, excel_dir: str = "checklists/excel"):
        self.excel_dir = get_checklists_path("excel")
    ...
    def update_app_config(self, config_path: str = "config/app_config.json"):
        ...
        try:
            # Use path utilities for config path
            actual_config_path = get_config_path("app_config.json")
    ...
    def update_roles_config(self, roles_path: str = "config/roles.json"):
        ...
        try:
            # Use path utilities for roles config path
            actual_roles_path = get_config_path("roles.json")
            
            # Load current roles config
            with open(actual_roles_path, 'r', encoding='utf-8') as f:
            ...
            with open(actual_roles_path, 'w', encoding='utf-8') as f:
