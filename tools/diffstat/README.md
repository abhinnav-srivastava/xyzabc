...
Place the **diffstat** native binary here. CodeCritique uses only **native executables** (e.g. `diffstat.exe` on Windows) so that no extra window or Java process starts when the app runs.
...
CodeCritique will still show diff stats: it uses a **pure-Python** fallback that parses the patch when the diffstat binary is not available. The binary is only needed if you want the exact `diffstat` output format.
