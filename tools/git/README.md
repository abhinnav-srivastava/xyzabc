...
Place a **portable Git** here so CodeCritique can use it for exact patch stats (GitLab-accurate) without requiring Git to be installed on the system.
...
CodeCritique will use `git` from the system **PATH**. Install Git normally and ensure `git` is on PATH if you want exact patch stats.
