        ...
        if 'Guidelines Description' in available_columns:
            ...
            for _, row in df.iterrows():
                ...
                if pd.notna(row.get('Guidelines Description', '')):
                    ...
                    # Add additional fields dynamically (only if columns exist)
                    if 'Good Example' in available_columns and pd.notna(row.get('Good Example', '')):
                        markdown_content += f"  - **Good Example:**\n```kotlin\n{row['Good Example']}\n```\n"
                    
                    if 'Bad Example' in available_columns and pd.notna(row.get('Bad Example', '')):
                        markdown_content += f"  - **Bad Example:**\n```kotlin\n{row['Bad Example']}\n```\n"
                    
                    if 'Measurement Reference' in available_columns and pd.notna(row.get('Measurement Reference', '')):
                        markdown_content += f"  - **Measurement:** {row['Measurement Reference']}\n"
                    
                    if 'External Refs' in available_columns and pd.notna(row.get('External Refs', '')):
            ...
            for _, row in df.iterrows():
                ...
                if pd.notna(row.get('Description', '')):
                    ...
                    # Add additional fields dynamically (only if columns exist)
                    if 'How to Measure' in available_columns and pd.notna(row.get('How to Measure', '')):
                        markdown_content += f"  - **How to Measure:** {row['How to Measure']}\n"
                    
                    if 'AdditionalInfo' in available_columns and pd.notna(row.get('AdditionalInfo', '')):
                        markdown_content += f"  - **Additional Info:** {row['AdditionalInfo']}\n"
                    
                    if 'Rule-Reference' in available_columns and pd.notna(row.get('Rule-Reference', '')):
